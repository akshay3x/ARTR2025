...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(	WS_EX_APPWINDOW,
							szAppName,
							TEXT("Vulkan| 04-Rectangle-02-Colored-Rectangle"),
...
void uninitialize(void)
{
	...
	if(vkDevice)
	{
		...
		if(vertexData_color.vkBuffer)
		{
			...
			fprintf(gpFile, "\nDEBUG:[uninitialize()][SUCCESS] vkDestroyBuffer() = vertexData_color.vkBuffer");
		}
		...
		if(vertexData_color.vkDeviceMemory)
		{
			...
			fprintf(gpFile, "\nDEBUG:[uninitialize()][SUCCESS] vkFreeMemory() = vertexData_color.vkDeviceMemory");
		}
...
VkResult createVertexBuffers(void)
{
	...
	//   Inside, declare our rectangle position array.
	...
	vkBufferCreateInfo.size = sizeof(rectangle_position); //size of the buffer in bytes
	...
	vkBufferCreateInfo.pNext = NULL;
	vkBufferCreateInfo.flags = 0; //valid flags are used in scatterd buffers/sparse buffers
	vkBufferCreateInfo.size = sizeof(rectangle_color); //size of the buffer in bytes
	vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; //VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
...
VkResult createPipeline(void)
{
	...
	vkVertexInputBindingDescription_array[1].stride = sizeof(float) * 3; 				//size of each vertex in bytes
	...
	vkVertexInputAttributeDescription_array[1].format = VK_FORMAT_R32G32B32_SFLOAT; 	//format of the color data
	vkVertexInputAttributeDescription_array[1].offset = 0; 								//offset of the color data
...
VkResult buildCommandBuffers(void)
{
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		//bind with color buffer
		VkDeviceSize vkDeviceSize_color;
		memset((void *)&vkDeviceSize_color, 0, sizeof(VkDeviceSize));

		//bind our desciptor set to the pipeline
		vkCmdBindDescriptorSets(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipelineLayout, 0, 1, &vkDescriptorSet, 0, NULL);

		vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 1, 1, &vertexData_color.vkBuffer, &vkDeviceSize_color);
		...
		//draw call
		vkCmdDraw(vkCommandBuffer_array[i], 6, 1, 0, 0); //draw call
