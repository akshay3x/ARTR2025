...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(	WS_EX_APPWINDOW,
							szAppName,
							TEXT("Vulkan | 05-Physical-Device"),
...
VkResult getPhysicalDevice(void)
{
	...
	if(bFound == VK_TRUE)
	{
		fprintf(gpFile, "\nDEBUG:[getPhysicalDevice][SUCCESS] Graphics enabled vkPhysicalDevice_Selected = %p", vkPhysicalDevice_Selected);
		...
		if(vkPhysicalDevice_array)
		{
			...
			fprintf(gpFile, "\nDEBUG:[getPhysicalDevice][SUCCESS] Free vkPhysicalDevice_array");
		}
	}
	else
	{
