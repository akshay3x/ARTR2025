...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(	WS_EX_APPWINDOW,
							szAppName,
							TEXT("Vulkan:Texture on Pyramid"),
	...
	float  pyramidTexcoords[] =
{
	...
	vkBufferCreateInfo.size = sizeof(pyramidVertices); //size of the buffer in bytes
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkCreateBuffer() vertexData_position.vkBuffer");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkCreateBuffer() vertexData_position.vkBuffer");
	}
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkAllocateMemory() vertexData_position.vkDeviceMemory");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkAllocateMemory() vertexData_position.vkDeviceMemory");
	}
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkBindBufferMemory() to vertexData_position.vkDeviceMemory");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkBindBufferMemory() to vertexData_position.vkDeviceMemory");
	}
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkMapMemory() to vertexData_position.vkDeviceMemory");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkMapMemory() to vertexData_position.vkDeviceMemory");
	}
	...
	fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] memcpy() pyramidVertices");
	...
	vkBufferCreateInfo.pNext = NULL;
	vkBufferCreateInfo.flags = 0; //valid flags are used in scatterd buffers/sparse buffers
	vkBufferCreateInfo.size = sizeof(pyramidTexcoords); //size of the buffer in bytes
	vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT; //VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
...
VkResult updateUniformBuffer(void)
{
	...
	rotationMatrix 					= glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(0.0f, 1.0f, 0.0f)); //rotate around Y axis
	modelMatrix 					= translationMatrix * rotationMatrix; //model matrix = translation * rotation
...
VkResult createDescriptorSet(void)
{
	...
	vkDescriptorSetAllocateInfo.descriptorSetCount 	= 1; 						//descriptor set count 
																				//though we have 2 descriptors ie. 1 for mvp uniforms and 1 for texture, 
																				//both are in one descriptor set
	...
	vkWriteDescriptorSet_array[0].dstSet 				= vkDescriptorSet; 					//descriptor set handle
	...
	vkWriteDescriptorSet_array[1].dstSet 				= vkDescriptorSet; 					//descriptor set handle
...
VkResult buildCommandBuffers(void)
{
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		//draw call
		vkCmdDraw(vkCommandBuffer_array[i], 12, 1, 0, 0); //draw call
