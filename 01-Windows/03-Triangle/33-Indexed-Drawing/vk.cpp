...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(	WS_EX_APPWINDOW,
							szAppName,
							TEXT("Vulkan| 03-Triangle-33-Indexed-Drawing"),
...
void uninitialize(void)
{
	...
	if(vkDevice)
	{
		...
		if(vertexData_position_index.vkBuffer)
		{
			...
			fprintf(gpFile, "\nDEBUG:[uninitialize()][SUCCESS] vkDestroyBuffer() = vertexData_position_index.vkBuffer");
		}
		...
		if(vertexData_position_index.vkDeviceMemory)
		{
			...
			fprintf(gpFile, "\nDEBUG:[uninitialize()][SUCCESS] vkFreeMemory() = vertexData_position_index.vkDeviceMemory");
		}
...
VkResult createVertexBuffers(void)
{
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkCreateBuffer() position index buffer");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkCreateBuffer() position index buffer");
	}
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkAllocateMemory() index buffer");
		return vkResult;
	}
	else
	{
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkBindBufferMemory() index buffer");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkBindBufferMemory() index buffer");
	}
	...
	if(vkResult != VK_SUCCESS)
	{	
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][ERROR] Failed vkMapMemory() index buffer");
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "\nDEBUG:[createVertexBuffers][SUCCESS] vkMapMemory() index buffer");
	}
	...
	fprintf(gpFile, "\nDEBUG:------------------------------------Done [createVertexBuffers()]------------------------------------\n");
	return vkResult;
}
/*--------------------------------------------------------------------------------------------------------*/
...
VkResult updateUniformBuffer(void)
{
	...
	myUniformData.modelMatrix 		= glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -4.0f)); //translate to -Z axis
...
VkResult buildCommandBuffers(void)
{
	...
	for(uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		//bind our index buffer to the pipeline
		vkCmdBindIndexBuffer(vkCommandBuffer_array[i], vertexData_position_index.vkBuffer, 0, VK_INDEX_TYPE_UINT32);	//fourth parameter is index type matching with triangle vertex indices type.
		...
		vkCmdDrawIndexed(vkCommandBuffer_array[i], numIndices, 1, 0, 0, 1); //draw call
