...
//validations
BOOL bValidationLayerSupport = FALSE;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(	WS_EX_APPWINDOW,
							szAppName,
							TEXT("Vulkan | 24-Descriptor-Set-Layout"),
...
VkResult initialize(void)
{
	...
	{	
		...
		fprintf(gpFile, "\nDEBUG:[initialize][SUCCESS] createVertexBuffers()");
	...
	{	
		...
		fprintf(gpFile, "\nDEBUG:[initialize][SUCCESS] createShaders()");
	...
	{	
		...
		fprintf(gpFile, "\nDEBUG:[initialize][SUCCESS] createDescriptorSetLayout()");	
...
//		if(vkDescriptorSet)
//		{
//			vkFreeDescriptorSets(vkDevice, vkDescriptorSetLayout, 1, &vkDescriptorSet);
...
VkResult fillInstanceExtensionNames(void)
{
	...
	for(uint32_t i = 0; i < instanceExtensionCount; i++)
	{
		...
		{
			fprintf(gpFile, "\nDEBUG:[fillInstanceExtensionNames][ERROR] VK_EXT_DEBUG_REPORT_EXTENSION_NAME found");
	...
	{
		if(bValidationLayerSupport == VK_TRUE)
			fprintf(gpFile, "\nDEBUG:[fillInstanceExtensionNames][ERROR] Validation is On but VK_EXT_DEBUG_REPORT_EXTENSION_NAME is Supported");
		else
			fprintf(gpFile, "\nDEBUG:[fillInstanceExtensionNames][INFO]  Validation is Off, VK_EXT_DEBUG_REPORT_EXTENSION_NAME is Supported");
	}
...
VkResult createDescriptorSetLayout(void)
{
	...
	//vkDescriptorSetLayoutBinding.binding					//binding index = 0;
	//vkDescriptorSetLayoutBinding.descriptorType  			//descriptor type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	//vkDescriptorSetLayoutBinding.descriptorCount			//descriptor count = 1; 
	//vkDescriptorSetLayoutBinding.stageFlags 				//shader stage flags = VK_SHADER_STAGE_VERTEX_BIT; 
	//vkDescriptorSetLayoutBinding.pImmutableSamplers		//sampler info = NULL; //not used in this example
